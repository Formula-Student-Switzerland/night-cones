...
int wifi_setup(void)
{
    ...
    {
        if (!WiFi.config(IPAddress(config_store.user_settings.ip_address),
                         IPAddress(config_store.user_settings.gateway),
                         IPAddress(config_store.user_settings.subnet)))
            Serial.print("Error in IP Address configuration!\n");
    }
...
uint32_t wifi_rx_handle_config(wifi_xtx_config_frame_t *rx_frame, uint16_t length)
{
    ...
    for (int i = 0; i < length; i += 2)
    {
        ...
        case WIFI_CONFIG_ID_TURNOFF_VOLTAGE:
            config_store.user_settings.turn_off_voltage_mv = value;
            break;
...
void wifi_tx_settings(IPAddress server_ip)
{
    ...
    wifi_cts_config_frame->data.values[WIFI_CONFIG_ID_TURNOFF_VOLTAGE] = config_store.user_settings.turn_off_voltage_mv;
    wifi_cts_config_frame->data.values[WIFI_CONFIG_ID_IP_ADDRESS] = config_store.user_settings.ip_address;
    wifi_cts_config_frame->data.values[WIFI_CONFIG_ID_IP_GATEWAY] = config_store.user_settings.gateway;
    wifi_cts_config_frame->data.values[WIFI_CONFIG_ID_IP_SUBNET] = config_store.user_settings.subnet;

    uint32_t *rst_info = (uint32_t *)ESP.getResetInfoPtr();
    for (int i = 0; i < 7; i++)
    {
