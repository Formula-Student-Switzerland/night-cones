...
\begin{table}[h!]
    ...
    \ReqItemMM{R:Time}{At maximum brightness the cone runs for the estimated time of a skidpad or acceleration event}{$\SI{4}{\minute}$}{N/A}
    ...
    \ReqItem{R:Usability}{An untrained person is capable of using the system without introduction. }
...
\todo[inline]{\ReqRef{R:Usability}: After initial setup? }
...
\begin{table}[h!]
    ...
    \ReqItem{R:Change}{An unmotivated of the color setting must be prevented. }
    ...
    \ReqItem{R:Rain}{The NightCone must be able to operate in rain. }
...
\todo[inline]{\ReqRef{R:Change}: Is unmotivated correctly translated? }
\todo[inline]{\ReqRef{R:Rain}: Specify ingress protection rating? (In this case IP54 or higher?) }
\todo[inline]{\ReqRef{R:Crash}: better word than deteriorated (beeinträchtigt)? }
